...
export class ExtractHarService {
   ...
   */
  async extractCurlFromHar(log: HarLog): Promise<string> {
    ...
    const curlText = await this.requestCurlFromOpenAi(minimal);
  ...
  private async requestCurlFromOpenAi(minimal: MinimalRequestSummary[]): Promise<string> {
    const client = this.getClient();
    const payload = JSON.stringify(minimal);
    const systemPrompt = `You are a tool that converts HTTP request data into curl commands.
...
- Use -H for each header. The URL already includes query string; use it as-is.`;
    ...
    const userMessage = `Convert these HTTP requests into curl commands. Output only the curl command(s), nothing else.\n\n${payload}`;
